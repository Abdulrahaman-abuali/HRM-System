...
features = df.groupby("employee_id").agg(
    ...
    avg_task_duration=("completed_at", lambda x: (x - df.loc[x.index,"start_date"]).dt.days.mean()),
    late_tasks=("completed_at", lambda x: ((x - df.loc[x.index,"due_date"]).dt.days > 0).sum()),
