...
<div class="main">
    <main class="main-content">
        <section class="section" style="margin-bottom: 1.5rem;">
            <article class="card">
                <div class="card-body">
                    <form method="GET" action="{{ route('users.index') }}" style="display: flex; gap: 1rem; align-items: flex-end; flex-wrap: wrap;">
                                    ...
                                    <tr style="border-bottom: 1px solid #edf2f7;">
                                        <td style="padding: 1rem;">
                                            <div style="display: flex; align-items: center; gap: 1rem;">
                                                <div style="width: 40px; height: 40px; border-radius: 10px; background: #eff6ff; color: #3b82f6; display: flex; align-items: center; justify-content: center; font-weight: bold; border: 1px solid #dbeafe;">
                                        ...
                                        <td style="padding: 1rem;">
                                            <span style="padding: 0.25rem 0.75rem; border-radius: 6px; font-size: 0.85rem; font-weight: 500;
                                        ...
                                        <td style="padding: 1rem;">
                                            <span style="display: inline-flex; align-items: center; gap: 0.4rem; padding: 0.25rem 0.6rem; border-radius: 999px; font-size: 0.8rem; font-weight: 600;
                                                {{ $user->status == 'نشط' ? 'background: #ecfdf5; color: #059669;' : 'background: #fef2f2; color: #dc2626;' }}">
                                                <span style="width: 8px; height: 8px; border-radius: 50%; background: currentColor;"></span>
                                                {{ $user->employee->status ?? 'غير محدد' }}
                                            </span>
                                        </td>
                                        <td style="padding: 1rem; text-align: center;">
                                            <div style="display: flex; gap: 0.5rem; justify-content: center;">
                                                <a href="{{ route('users.edit', $user->id) }}" class="btn btn-sm btn-outline" title="تعديل الصلاحيات">
                                                    إدارة الصلاحية
                                                </a>
                                                <form action="{{ route('users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف الحساب؟ لن يتم حذف بيانات الموظف.')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-sm btn-danger" style="background: #fee2e2; color: #dc2626; border: none;">حذف</button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
