        ...
        <div class="auth-wrapper" style="max-width: 800px; margin: auto;">
            <div class="auth-card">
                <header class="auth-header" style="text-align: center; margin-bottom: 2rem;">
                    ...
                    <p class="brand-subtitle">تحديث بيانات: <strong>{{ $employee->first_name }} {{ $employee->last_name }}</strong></p>
                </header>

                <form action="{{ route('employees.update', $employee->id) }}" method="POST" class="auth-form" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="phone" name="phone" class="form-control" value="{{ old('phone', $employee->phone) }}" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" id="age" name="age" class="form-control" value="{{ old('age', $employee->age) }}" readonly placeholder="يتم حسابه تلقائياً">
                        </div>
                    </div>
                    ...
                    <div class="form-group" style="grid-column: span 2; margin-bottom: 1.5rem;">
                        ...
                        <div style="display: flex; align-items: center; gap: 1rem; border: 2px dashed #e5e7eb; padding: 1rem; border-radius: 8px;">
                            <input type="file" id="profile_image" name="profile_image" class="form-control" accept="image/*" onchange="previewImage(event)">
                            <div id="image_preview_container" style="width: 80px; height: 80px; border: 1px solid #ddd; border-radius: 50%; overflow: hidden; background: #f9fafb; display: flex; align-items: center; justify-content: center;">
                                @if($employee->profile_image)
                                    <img id="output" src="{{ asset('storage/employee_faces/' . $employee->profile_image) }}" style="width: 100%; height: 100%; object-fit: cover;"/>
                                @else
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
                        ...
                        <div class="form-group">
                            ...
                            <select id="employment_type" name="employment_type" class="form-control" required>
                                ...
                                <option value="contract" {{ old('employment_type', $employee->employment_type) == 'contract' ? 'selected' : '' }}>تعاقد / فريلانس</option>
                            </select>
                        </div>
                        ...
                        {{-- حقول التعاقد --}}
                        <div id="contract_dates_container" style="display: {{ old('employment_type', $employee->employment_type) == 'contract' ? 'grid' : 'none' }}; grid-column: span 2; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 1rem; background: #f9fafb; padding: 1rem; border-radius: 8px; border: 1px solid #e5e7eb;">
                            <div class="form-group">
                                <label for="contract_start_date" class="form-label">تاريخ بداية العقد</label>
                                <input type="date" id="contract_start_date" name="contract_start_date" class="form-control" value="{{ old('contract_start_date', $employee->contract_start_date) }}">
                            </div>
                            <div class="form-group">
                                <label for="contract_end_date" class="form-label">تاريخ نهاية العقد</label>
                                <input type="date" id="contract_end_date" name="contract_end_date" class="form-control" value="{{ old('contract_end_date', $employee->contract_end_date) }}">
                            </div>
                        </div>

                        <div class="form-group" style="grid-column: span 2;">
                            <label for="basic_salary" class="form-label">الراتب الأساسي (ر.ي)</label>
                            <input type="number" step="0.001" id="basic_salary" name="basic_salary" class="form-control"
    ...
    document.getElementById('department_id').addEventListener('change', function() {
        ...
        if (deptId) {
            fetch(`/get-managers/${deptId}`)
                ...
                .then(data => {
                    ...
                    if (data.length > 0) {
                        ...
                        managerSelect.innerHTML = '<option value="" selected>بدون مدير (لا يوجد مديرين)</option>';
                    }
                });
            ...
            fetch(`/check-section-manager/${deptId}`)
                .then(response => response.json())
                .then(data => {
                    const isCurrentManager = "{{ $employee->jobTitle->name ?? '' }}".includes('مدير') && "{{ $employee->department_id }}" == deptId;
