    ...
    <section class="section">
        <div class="auth-wrapper" style="max-width: 800px; margin: auto;">
            <div class="auth-card">
                ...
                <form action="{{ route('employees.store') }}" method="POST" class="auth-form" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
                        ...
                        <div id="contract_dates_container" style="display: none; grid-column: span 2; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 1rem; background: #f9fafb; padding: 1rem; border-radius: 8px; border: 1px solid #e5e7eb;">
                            <div class="form-group">
                                <label for="contract_start_date" class="form-label">تاريخ بداية العقد</label>
                                <input type="date" id="contract_start_date" name="contract_start_date" class="form-control" value="{{ old('contract_start_date') }}">
                            </div>
                            <div class="form-group">
                                <label for="contract_end_date" class="form-label">تاريخ نهاية العقد</label>
                                <input type="date" id="contract_end_date" name="contract_end_date" class="form-control" value="{{ old('contract_end_date') }}">
                            </div>
                        </div>

                        <div class="form-group">
                            <label for="basic_salary" class="form-label">الراتب الأساسي (ر.ي)</label>
                            <input type="number" step="0.001" id="basic_salary" name="basic_salary" class="form-control" value="{{ old('basic_salary') }}" required>
                        </div>
                    </div>
                ...
                .then(data => {
                    ...
                    if (data.length > 0) {
                        data.forEach((manager, index) => {
                            ...
                            option.textContent = manager.first_name + ' ' + manager.last_name + ' (مدير القسم)';
                            if (index === 0) option.selected = true;
                            managerSelect.appendChild(option);
                        });
                    } else {
                        managerSelect.innerHTML = '<option value="" selected>لا يوجد مدير لهذا القسم</option>';
                    }
                });
